...
def get_expenses(expense_date: date):
    ...
    if expenses is None:
        raise HTTPException(status_code=500, detail="Failed  to retrive expenses from the database")
    return expenses
...
def add_or_update_expense(expense_date: date, expenses:List[Expense]):
    ...
    return {"message": "Expense Updated Sucessfully"}
...
def get_analytics(date_range:DateRange):
    ...
    if data is None:
        raise HTTPException(status_code=500, detail="Failed  to retrive expense summary from the database")
